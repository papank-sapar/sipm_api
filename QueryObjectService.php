...
class QueryObjectService {
	...
	private function addQueryLine ($rel, $col, $test, $value) {
		...
		if (!in_array($test, array('!=', '>=', '<=', '<', '>', '=', 'contains', 'not_null', 'is_null'))) {
		...
		$test = str_replace(array('!=', '>=', '<=', '<', '>', '=', 'contains', 'not_null', 'is_null'), array('Neq', 'Geq', 'Leq', 'Lt', 'Gt', 'Eq', 'Contains', 'IsNotNull', 'IsNull'), $test);

		// Create caml
		$caml = $this->where_caml;
		if ($test === 'IsNotNull' || $test === 'IsNull') {
			$content = '<FieldRef Name="' . $col . '" />' . PHP_EOL;
		} else {
